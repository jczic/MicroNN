...
class MicroNN :
    ...
    class ActFunctions :
        ...
        def Heaviside(x, derivative=False) :
            if derivative :
                return 0.0 if x != 0 else 0.54321
    ...
    def LearnExamples( self,
        ...
        count = 0
        if self.ExamplesCount > 0 :
            endTime = time() + maxSeconds
            while time() < endTime and \
                  ( maxCount is None or count < maxCount ) :
                idx = int( MicroNN.RandomFloat() * self.ExamplesCount )
                self.Learn(self._examples[idx][0], self._examples[idx][1])
                count += 1
                if (stopWhenLearned or printMAEAverage) and count % 100 == 0 :
                    successAvg = 0.0
                    for ex in self._examples :
                        successAvg += self.Test(ex[0], ex[1])
                    successAvg /= self.ExamplesCount
                    if verbose :
